...
import midas.targetutils.SynthesizePrintf
    ...
    /*
    Attempt at request level

    //Begin Modification

    // 1) Tap off the packed beats
    ...
    beatPacker.io.req.bits.lg_len_req := core.module.io.beatData.bits.lg_len_req //required?
    beatPacker.io.in.valid  := core.module.io.beatData.valid
    beatPacker.io.in.bits   := core.module.io.beatData.bits.data

    // 2) FIFO the BeatMerger outputs
    val beatType = beatPacker.io.out.bits.cloneType
    val beatFifo = Module(new Queue(beatType, entries = 4, pipe = true))
    beatFifo.io.enq <> beatPacker.io.out

    // 3) Split into data / meta FIFOs
    val dataFifo = Module(new Queue(UInt(beatBits.W), entries = 4, pipe = true))
    val metaFifo = Module(new Queue(beatType, entries = 4, pipe = true))

    dataFifo.io.enq.bits  := beatFifo.io.deq.bits.data
    dataFifo.io.enq.valid := beatFifo.io.deq.valid
    metaFifo.io.enq.bits  := beatFifo.io.deq.bits
    metaFifo.io.enq.valid := beatFifo.io.deq.valid
    beatFifo.io.deq.ready := dataFifo.io.enq.ready && metaFifo.io.enq.ready

    when (dataFifo.io.enq.fire) {
          printf(":DMA:BEATDATA:  DATA(0x%x)\n",
      beatFifo.io.deq.bits.data)
    }

    // 4) AES wrapper on just the 128b data + carry along the beat‑meta
    val aes_key_reg = RegInit("h1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100".U(256.W)) //Corrected for endianness
    val aes = Module(new AESWrapper(beatBits, beatType))
    aes.io.key     := aes_key_reg
    aes.io.encrypt := false.B  // decrypt

    aes.io.in.valid       := dataFifo.io.deq.valid  && metaFifo.io.deq.valid
    aes.io.in.bits.data   := dataFifo.io.deq.bits
    aes.io.in.bits.meta   := metaFifo.io.deq.bits
    dataFifo.io.deq.ready := aes.io.in.fire
    metaFifo.io.deq.ready := aes.io.in.fire

    // 5) Re‑assemble into io.resp
    aes.io.out.ready := io.resp.ready
    io.resp.valid   := aes.io.out.valid

    // data comes back decrypted
    io.resp.bits.data        := aes.io.out.bits.data

    // Not in original
    // io.resp.bits.xactid      := aes.io.out.bits.meta.xactid
    // io.resp.bits.lg_len_req  := aes.io.out.bits.meta.lg_len_req

    // now fill in the rest of your response fields from the same meta bundle:
    io.resp.bits.addr        := aes.io.out.bits.meta.addr
    io.resp.bits.mask        := aes.io.out.bits.meta.mask
    io.resp.bits.is_acc      := aes.io.out.bits.meta.is_acc
    io.resp.bits.accumulate  := aes.io.out.bits.meta.accumulate
    io.resp.bits.has_acc_bitwidth := aes.io.out.bits.meta.has_acc_bitwidth
    // These aren't orginially coming out of beatPacker
    io.resp.bits.scale := RegEnable(xactTracker.io.peek.entry.scale, aes.io.out.fire) // io.resp.bits.scale       := aes.io.out.bits.meta.scale
    io.resp.bits.repeats := RegEnable(xactTracker.io.peek.entry.repeats, aes.io.out.fire) // io.resp.bits.repeats     := aes.io.out.bits.meta.repeats
    io.resp.bits.pixel_repeats := RegEnable(xactTracker.io.peek.entry.pixel_repeats, aes.io.out.fire) // io.resp.bits.pixel_repeats := aes.io.out.bits.meta.pixel_repeats
    io.resp.bits.len := RegEnable(xactTracker.io.peek.entry.len, aes.io.out.fire) // io.resp.bits.len         := aes.io.out.bits.meta.len
    io.resp.bits.cmd_id := RegEnable(xactTracker.io.peek.entry.cmd_id, aes.io.out.fire) // io.resp.bits.bytes_read  := aes.io.out.bits.meta.bytes_read
    io.resp.bits.bytes_read := RegEnable(xactTracker.io.peek.entry.bytes_to_read, aes.io.out.fire) // io.resp.bits.cmd_id      := aes.io.out.bits.meta.cmd_id
    io.resp.bits.last        := aes.io.out.bits.meta.last
...
class StreamReaderCore[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V], nXacts: Int, beatBits: Int,
  ...
  // Buffers
  val aes_data_reg   = Reg(UInt(128.W))
  val aes_meta_reg   = Reg(new StreamReadBeat(nXacts,beatBits,maxBytes))
  val aes_data_valid = RegInit(false.B)

  val maxPipelinedBeats = 16
  val inFlightAES = RegInit(0.U(log2Ceil(maxPipelinedBeats+1).W))

  // AES Wrapper
  val aes_wrapper = Module(new AESWrapper(128, new StreamReadBeat(nXacts,beatBits,maxBytes)))
  aes_wrapper.io.key     := aes_key_reg
  aes_wrapper.io.encrypt := false.B

  // Capture Beat
  val beat_is_128b = tl.d.bits.size === log2Ceil(16).U
  val new_beat_valid = tl.d.valid && beat_is_128b
  // only capture if wrapper is ready and we have no beat already pending
  val capture_beat = new_beat_valid && !aes_data_valid && aes_wrapper.io.in.ready

  // back‑pressure TL until we can accept a new beat
  tl.d.ready := !aes_data_valid && aes_wrapper.io.in.ready

  when (capture_beat) {
    aes_data_reg   := tl.d.bits.data
    aes_meta_reg.xactid     := tl.d.bits.source
    aes_meta_reg.lg_len_req := tl.d.bits.size
    aes_meta_reg.last       := edge.last(tl.d)
    aes_data_valid := true.B

    printf(":DMA:READ:DECRYPT:TILELINK  DATA(0x%x) XACTID(0x%x)\n",
      tl.d.bits.data, tl.d.bits.source)
  }

  // Drive AES from buffers
  aes_wrapper.io.in.valid := aes_data_valid
  aes_wrapper.io.in.bits.data := aes_data_reg
  aes_wrapper.io.in.bits.meta := aes_meta_reg

  // Clear flag on fire
  when (aes_wrapper.io.in.fire) {
    aes_data_valid := false.B
    printf(":DMA:READ:DECRYPT:IN    Data(0x%x)\n", aes_data_reg)
  }

  // For performance measuring
  when (aes_wrapper.io.out.fire) {
    printf(":DMA:READ:DECRYPT:OUT   Data(0x%x)\n", aes_wrapper.io.out.bits.data)
  }

  // To account for +- same cycle
  val inc = aes_wrapper.io.in.fire.asUInt
  val dec = aes_wrapper.io.out.fire.asUInt
  inFlightAES := inFlightAES + inc - dec
  val aesBusy = inFlightAES =/= 0.U

  // hook up the beatData channel
  aes_wrapper.io.out.ready := io.beatData.ready
  io.beatData.valid        := aes_wrapper.io.out.valid
  io.beatData.bits.data    := aes_wrapper.io.out.bits.data
  io.beatData.bits.xactid  := aes_wrapper.io.out.bits.meta.xactid
  io.beatData.bits.lg_len_req := aes_wrapper.io.out.bits.meta.lg_len_req
  io.beatData.bits.last    := aes_wrapper.io.out.bits.meta.last
  ...
  //Original Code:
    ...
    io.counter.connectEventSignal(CounterEvent.RDMA_ACTIVE_CYCLE, state =/= s_idle && !aesBusy) //Questionable now
    ...
    // New AES‐stall cycle counter
    // Count a cycle whenever TileLink has valid data but we can’t hand it to the AES wrapper,
    // *or* whenever the AES wrapper has decrypted data ready but downstream isn’t ready. (Shouldn't happen)
    val aesReadStall  = tl.d.valid  && !aes_wrapper.io.in.ready
    val aesWriteStall = aes_wrapper.io.out.valid && !io.beatData.ready
    val aesStall = aesReadStall || aesWriteStall
...
class StreamWriter[T <: Data: Arithmetic](nXacts: Int, beatBits: Int, maxBytes: Int, dataWidth: Int, aligned_to: Int,
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters with MemoryOpConstants {
    ...
    // New request with putFull or putPartial
    val untranslated_a = Wire(Decoupled(new TLBundleAWithInfo))

    val beatData = (data >> (bytesSent * 8.U)).asUInt

    // AES Registers
    val aes_key_reg = RegInit("h1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100".U(256.W))
    val aes_data_reg   = Reg(UInt(128.W))
    val aes_data_valid = RegInit(false.B)
    val new_beat = RegInit(true.B)

    // Performance Counter
    //val aesBusy = RegInit(false.B)

    val encryptComplete = RegInit(false.B)
    val encryptedBeat   = Reg(UInt(128.W))
    ...
    // Capture Beat of 16 Bytes
    val beat_is_128b = true.B //assumed in for tested instructions

    // Possible Logic
    // val beat_len_when_new = bestWP.bytes_written_per_beat(0)
    // val beat_is_128b    = (state === s_writing_new_block  &&  beat_len_when_new === 16.U) ||
    //                    (state =/= s_writing_new_block &&  write_packet.bytes_written_per_beat(beatsSent) === 16.U)

    val new_beat_valid = (state === s_writing_new_block || state === s_writing_beats) && beat_is_128b
    val capture_beat = new_beat_valid && !aes_data_valid && new_beat


    // A simple cycle counter as a sanity check
    ...
    when (capture_beat) {
      aes_data_reg   := beatData
      aes_data_valid := true.B
      new_beat := false.B
      
      printf(":DMA:WRITE:ENCRYPT: Beat Captured\n")
      //aesBusy := true.B
      cycle := 0.U
    }

    //Between capturing a beat and untranslated_a.fire we do not accept another beat

    // AES IN
    aes.io.in.valid        := aes_data_valid
    aes.io.in.bits.data    :=  aes_data_reg
    aes.io.out.ready       := !encryptComplete //true.B //Gate with untranslated_a being ready somehow
    ...
    when (aes.io.in.fire) {
      aes_data_valid := false.B
      printf(":DMA:WRITE:ENCRYPT:IN: Data(0x%x), @:(%d)\n", aes_data_reg, cycle)
    }

    // encrypted data is produced
    when (aes.io.out.fire) {
      encryptedBeat := aes.io.out.bits.data
      encryptComplete := true.B
      printf(":DMA:WRITE:ENCRYPT:OUT Data(0x%x), @:(%d)\n", aes.io.out.bits.data, cycle)
    }


    //PERFORMANCE VALS

    // 1) Stall: you're valid => core not ready
    val aesWriteStall = aes.io.in.valid && !aes.io.in.ready

    // 2) Busy: core has accepted but not yet produced

    inFlightAES := inFlightAES + aes.io.in.fire.asUInt - aes.io.out.fire.asUInt

    // Busy whenever there’s at least one beat juggling through AES
    val aesBusy = inFlightAES =/= 0.U

    // in your StreamWriter.Impl, after wiring aes.io.in / aes.io.out:
    // val setBusy   = aes.io.in.fire
    // val clearBusy = aes.io.out.fire

    // // Reg that stays high from the cycle you set it until the *next* cycle you clear it
    // val aesBusy = RegInit(false.B)
    // aesBusy := Mux(setBusy,   true.B,
    //           Mux(clearBusy, false.B,
    //                         aesBusy))


    //between when the AES output and untranslated_a.fire we are not ready to accept another AES ouput

    //encrypted data consumed
    when (untranslated_a.fire) {
      printf(":DMA:WRITE:ENCRYPT: Beat Consumed\n")
      new_beat := true.B
      encryptComplete := false.B
    }
    ...
    val putFull = edge.Put(
      ...
      //Natural AES Encryption spot
      data = encryptedBeat.asUInt
      //AES Encryption
    )._2
    ...
    val putPartial = edge.Put(
      ...
      lgSize = lg_write_size,
      //AES Encryption
      data = (encryptedBeat << (write_shift * 8.U)).asUInt,
      //AES Encryption
    ...
    // New request with putFull or putPartial
    //val untranslated_a = Wire(Decoupled(new TLBundleAWithInfo))
    // On new block, record xact id as busy
    xactBusy_fire := untranslated_a.fire && state === s_writing_new_block
    //MODIFIED: untranslated_a.valid := (state === s_writing_new_block || state === s_writing_beats) && !xactBusy.andR
    untranslated_a.valid := (state === s_writing_new_block || state === s_writing_beats) && !xactBusy.andR && encryptComplete
    ...
    // 0 goes to retries, 1 goes to state machine
    // TLB Miss or need Retried
    val retry_a = Wire(Decoupled(new TLBundleAWithInfo))
    // Request waiting for bus to be available
    ...
    // Prioritize TLB Miss, Bus Busy, then New Reuest
    ...
    // Whichever Request Wins, use TLB for to get page
    val tlb_q = Module(new Queue(new TLBundleAWithInfo, 1, pipe=true))
    tlb_q.io.enq <> tlb_arb.io.out
    ...
    // Stash full request until TLB responds
    val translate_q = Module(new Queue(new TLBundleAWithInfo, 1, pipe=true))
    translate_q.io.enq <> tlb_q.io.deq
    when (retry_a.valid) {
    ...
    // If TLB Hit
    tl.a.valid := translate_q.io.deq.valid && !io.tlb.resp.miss
    // Send queued request with physical address
    tl.a.bits := translate_q.io.deq.bits.tl_a
    ...
    when (untranslated_a.fire) {
      when (state === s_writing_new_block) {
        ...
        printf(":DMA:WRITE:NEWBLOCK Data(0x%x), Source(0x%x), Address(0x%x), Size(0x%x), WriteFull(0x%x), BytesSent (0x%x), BytesThisBeat(0x%x)\n", 
          untranslated_a.bits.tl_a.data,
          untranslated_a.bits.tl_a.source, 
          untranslated_a.bits.vaddr, 
          untranslated_a.bits.tl_a.size,
          write_full, 
          bytesSent, 
          bytes_written_this_beat)
        ...
        printf(":DMA:WRITE:WRITEBEATS Data(0x%x), Source(0x%x), Address(0x%x), Size(0x%x), WriteFull(0x%x), BytesSent (0x%x), BytesThisBeat(0x%x)\n", 
          untranslated_a.bits.tl_a.data,
          untranslated_a.bits.tl_a.source, 
          untranslated_a.bits.vaddr, 
          untranslated_a.bits.tl_a.size, 
    ...
    // untranslated_a.valid when everything else is true, but is waiting on AES encryptComplete
    io.counter.connectEventSignal(CounterEvent.WDMA_AES_STALL_CYCLES, aesWriteStall)//(state === s_writing_new_block || state === s_writing_beats) && !xactBusy.andR && !encryptComplete)
    ...
    total_latency := total_latency + PopCount(xactBusy) //+ aesBusy.asUInt //Interesting how latency is calculated here
...
class StreamWriter[T <: Data: Arithmetic](nXacts: Int, beatBits: Int, maxBytes: Int, dataWidth: Int, aligned_to: Int,
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters with MemoryOpConstants {
    ...
    val aes_key_reg = RegInit("h1f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100".U(256.W))
    // === new: 1) buffer a full 16‑byte block at the start of each block ===
    val aes_block_buf   = Reg(UInt(128.W))
    val aes_block_valid = RegInit(false.B)

    when(state === s_writing_new_block && beatsSent === 0.U && !aes_block_valid) {
      aes_block_buf   := data // your full 128‑bit word
      aes_block_valid := true.B
    }

    // === new: 2) drive the AES engine ===
    val aes = Module(new AESWrapper(128, new Packet()))
    aes.io.key     := aes_key_reg
    aes.io.encrypt := true.B

    // only fire once per block
    aes.io.in.valid := aes_block_valid && aes.io.in.ready
    aes.io.in.bits.data := aes_block_buf
    aes.io.in.bits.meta := DontCare  // you don’t need per‐beat meta here

    // once AES has accepted it, clear the valid flag
    when(aes.io.in.fire) { 
      aes_block_valid := false.B 
      printf(":DMA:Encrypt:AES_IN Data(0x%x), BytesThisBeat(0x%x)\n", aes_block_buf, bytes_written_this_beat)
      }

    // capture the result
    val aes_block_out = Reg(UInt(128.W))
    aes.io.out.ready := true.B
    when(aes.io.out.fire) { 
      aes_block_out := aes.io.out.bits.data 
      printf(":DMA:Encrypt:AES_OUT Data(0x%x), FullBeat(0x%x)\n", aes_block_out, encrypted_beat)
      }

    // === new: 3) piece your beat back out of aes_block_out ===
    // this replaces your existing `raw_beat_data >> (write_shift*8.U)`
    val encrypted_beat = (aes_block_out >> (write_shift*8.U)).asUInt

    // === finally, your existing TL Put, but with encrypted_beat instead of raw data ===
    val putFull = edge.Put(
      fromSource = RegEnableThru(xactId, state === s_writing_new_block),
      toAddress  = 0.U,
      lgSize     = lg_write_size,
    ...
    // val aes = Module(new AesCipherCoreDriver)
    // aes.io.in.bits.encrypt := true.B
    // aes.io.in.bits.key := aes_key_reg // Key is assumed preloaded via config

    // // Subset of data to be encrypted this beat
    // val current_beat_data = (data >> (bytesSent * 8.U)).asUInt

    // // AES handshaking
    // aes.io.in.valid := (state === s_writing_new_block || state === s_writing_beats) && !xactBusy.andR
    // aes.io.in.bits.data := current_beat_data

    // when (aes.io.in.fire) {
    //   printf(":DMA:Encrypt: Data(0x%x), BytesThisBeat(0x%x)\n", current_beat_data, bytes_written_this_beat)
    // }

    // val aes_ready = aes.io.out.valid
    // aes.io.out.ready := tl.a.ready

    // val encrypted_data = aes.io.out.bits.data
    ...
    //   lgSize = lg_write_size,
    //   //AES Encryption
    //   data = (encrypted_data << (write_shift * 8.U)).asUInt,
    //   //AES Encryption
    //   mask = write_mask.asUInt
    // )._2
...
*/
    ...
    // MODIFIED
    tl.a.valid := translate_q.io.deq.valid && !io.tlb.resp.miss
    //tl.a.valid := translate_q.io.deq.valid && !io.tlb.resp.miss && aes_ready
    // MODIFIED END
